    ...
    def MainFunc(self, content_path, style_path, iter, resolution):
        ...
        def load_and_process_img(path_to_img, flag = 0, image= None):
            ...
            if flag == 1:
                ...
                cnn = CNN()
                model= cnn.Build()
                np_image = Image.open(path_to_img)
                np_image = np.array(np_image).astype('float32') / 255
                np_image = transform.resize(np_image, (256, 256, 3))
                np_image = np.expand_dims(np_image, axis=0)
                img = model.predict(np_image)
...
class CNN(tf.keras.Model):
    ...
    def Build(self):
        model = Sequential()
        model.add(Conv2D(32, (9,9),strides=(1,1), padding='same', activation = None ) )
        model.add(layers.BatchNormalization())
        model.add(layers.Activation('relu'))

        model.add(Conv2D(64,kernel_size=(3,3),strides=(2,2),padding='same',activation=None))
        model.add(layers.BatchNormalization())
        model.add(Activation('relu'))

        model.add(Conv2D(128,kernel_size=(3,3),strides=(2,2),padding='same',activation=None))
        model.add(layers.BatchNormalization())
        model.add(Activation('relu'))
        ...
        model.add(layers.Conv2DTranspose(64, kernel_size = (3, 3), strides=(2,2),padding='same', activation = None))
        model.add(layers.BatchNormalization())
        model.add(Activation('relu'))

        model.add(layers.Conv2DTranspose(32, kernel_size = (3, 3), strides=(2,2),padding='same', activation = None))
        model.add(layers.BatchNormalization())
        model.add(Activation('relu'))

        model.add(Conv2D(3, kernel_size = (9, 9),strides=(1,1), padding='same', activation= None))
        model.add(layers.BatchNormalization())
